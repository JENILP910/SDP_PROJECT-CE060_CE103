...
const AudioPlayer = ({ currentSongs}) => {
	...
	// const { currentPlayList, currentSong } = useSelector((state) => state.audioPlayer);
	// const { user, likeSongProgress } = useSelector((state) => state.user);
	const dispatch = useDispatch();

// console.log("currentSong-->");
// console.log(currentSong.sId);
// console.log(currentSong.song);
// console.log(currentSong.playlist);
// console.log(currentSong.song.song);
	// const { song } = sng[trackIndex];

	// const audioRef = useRef(new Audio("https://firebasestorage.googleapis.com/v0/b/online-music-streaming-a-486c7.appspot.com/o/audio%2F1646454828077Hypnotic.mp3?alt=media&token=f5b31412-5d93-46a7-a1c0-5d0cb9f4af26"));
	const audioRef = useRef(new Audio(currSong));
	// const audioRef = useRef(new Audio(currentSong.song.song));
	...
	if(trackIndex + 1 > numOfSongs)
		...
		setTrackIndex(trackIndex+1);
	audioRef.current = new Audio(currentSong.playlist[trackIndex]);
		...
		<div className={styles.audio_player}>
			...
			<div className={styles.center}>
				...
				<div className={styles.progress_container}>
					...
					<p>{duration ? (Math.floor(duration) > 60 ? (parseInt(Math.floor(duration)/60) + ":" + Math.floor(duration)%60) : Math.floor(duration)) : 0}</p>
				</div>
			</div>
